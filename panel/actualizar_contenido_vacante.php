...
if($_SESSION['permisos']!='1' && $_SESSION['permisos']!='4' ){
  ...
  {  
         $id_p = $_GET['id_p'];
         $id_p = base64_decode($id_p);
 }

  $consulta_parrafo = $cone->prepare("select * from parrafo_empleo where id_p = '".$id_p."'"); 
  $consulta_parrafo->execute();
  $parrafo = $consulta_parrafo->fetch(PDO::FETCH_ASSOC);
?>
            ...
            <form  id="actualizar_contenido_vacante" action="" method="post" class="mx-3">
                <label ><b>TEXTO EN ESPAÑOL</b></label>
                <textarea id="editor" name="editor"><?php echo $parrafo['parrafo']?></textarea>

                <label class="mt-5"><b>TEXTO EN INGLÉS</b></label>
                <textarea id="editori" name="editori"><?php echo $parrafo['parrafoi']?></textarea>
                <button class="btn btn-primary my-3" type="submit">Actualizar parrafo</button>
                <input type="text" name="id_p" id="id_p" value="<?php echo $id_p?>" style="opacity:0%;">
            </form>
    ...
    document.getElementById('actualizar_contenido_vacante').addEventListener('submit', function(event) {
